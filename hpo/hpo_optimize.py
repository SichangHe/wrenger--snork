...
def snake_from_config_wrapper(type, num_opponents, num_games_per_eval, timeout):
    def snake_from_config(cfg, *args, **kwargs):
        ...
        for rs in RANDOMSTATES:
            ...
            run = subprocess.run(
                call, capture_output=True, shell=True, text=True, check=True
            )
        ...
        if type == "DEHB":
            return {"fitness": fitness, "cost": time_used, "info": {}}
...
def get_cs(agentlist=["Flood"]):
    ...
    if "Mobility" in agentlist:
        mobility_hps = {
            "health_threshold": UniformIntegerHyperparameter(
                PREFIXES["Mobility"] + "health_threshold",
                lower=0,
                upper=100,
                default_value=35,
            ),
            "min_len": UniformIntegerHyperparameter(
                PREFIXES["Mobility"] + "min_len", lower=0, upper=121, default_value=8
            ),
            "first_move_cost": UniformFloatHyperparameter(
                PREFIXES["Mobility"] + "first_move_cost",
                lower=0,
                upper=3,
                default_value=1.0,
            ),
    ...
    if "Flood" in agentlist:
        ...
        for hp in flood_hps:
            flood_hps[hp] = UniformFloatHyperparameter(
                PREFIXES["Flood"] + hp,
...
def optim_smac(args):
    ...
    smac = SMAC4HPO(
        scenario=scenario, rng=np.random.RandomState(RANDOMSTATES[0]), tae_runner=tae
    )
