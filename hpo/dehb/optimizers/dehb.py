...
class DEHBBase:
    def __init__(
        ...
        **kwargs,
        ...
        ), "only (Max Budget > Min Budget) supported!"
        self.eta = eta
        ...
        self.logger.add(
            "{}/dehb_{}.log".format(self.output_path, log_suffix), **_logger_props
        )
...
class DEHB(DEHBBase):
    ...
    def _update_trackers(self, traj, runtime, history):
        ...
        self.smac_history["data"].append(
            (
                [
                    len(self.traj) + 1,  # config id
                    None,  # instance id
                    0,  # seed...
                    history[-2],  # budget
                ],
                [
                    ...
                    history[2],  # time / cost
                    {"__enum__": "StatusType.SUCCESS"},  # status
    ...
    def _init_subpop(self):
        ...
        for i, b in enumerate(self._max_pop_size.keys()):
            self.de[b] = AsyncDE(
                **self.de_params, budget=b, pop_size=self._max_pop_size[b]
            )
    ...
    def _get_promotion_candidate(self, low_budget, high_budget, n_configs):
        ...
        if (
            self.de[high_budget].promotion_pop is None
            or len(self.de[high_budget].promotion_pop) == 0
        ):
            ...
            for idx in pop_idx:
                ...
                if np.any(
                    np.all(individual == self.de[high_budget].population, axis=1)
                ):
            ...
            self.de[high_budget].promotion_pop = self.de[high_budget].promotion_pop[
                :n_configs
            ]
    ...
    def _get_next_parent_for_subpop(self, budget):
        ...
        self.de[budget].parent_counter = (
            self.de[budget].parent_counter % self._max_pop_size[budget]
        )
        return parent_id
    ...
    def _acquire_config(self, bracket, budget):
        ...
        if self.iteration_counter < self.max_SH_iter:
            ...
            if budget != bracket.budgets[0]:
                ...
                config = self._get_promotion_candidate(
                    lower_budget, budget, num_configs
                )
    ...
    def _get_gpu_id_with_low_load(self):
        ...
        gpu_ids = self._create_cuda_visible_devices(self.available_gpus, device_id)
        # updating GPU usage
    ...
    def _fetch_results_from_workers(self):
        ...
        for _, future in done_list:
            ...
            self._update_trackers(
                traj=(self.inc_score, self.inc_config),
                runtime=cost,
                history=(config, float(fitness), float(cost), float(budget), info),
            )
    ...
    def _save_incumbent(self, name=None):
        ...
        try:
            ...
            with open(
                os.path.join(self.output_path, "incumbent_{}.json".format(name)), "w"
            ) as f:
                json.dump(res, f)
    ...
    def _save_history(self, name=None):
        ...
        try:
            with open(
                os.path.join(self.output_path, "history_{}.pkl".format(name)), "wb"
            ) as f:
                pickle.dump(self.history, f)
            if self.save_smac:
                with open(
                    os.path.join(self.output_path, "runhistory_{}.json".format(name)),
                    "w",
                ) as f:
                    json.dump(self.smac_history, f, indent=2)
                with open(
                    os.path.join(self.output_path, "traj_{}.json".format(name)), "w"
                ) as f:
    ...
    def _verbosity_debug(self):
        for bracket in self.active_brackets:
            self.logger.debug(
                "Bracket ID {}:\n{}".format(bracket.bracket_id, str(bracket))
            )
